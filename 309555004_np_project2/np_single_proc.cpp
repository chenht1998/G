        ...
        for(int fd=0; fd<nfds; fd++){
            if(FD_ISSET(fd, &readFds) && fd!=msockfd){
                ...
                //handle data coming from client
                    ...
                    ////kick [id]
                        int kicked_userID = atoi(Cmds[0].CmdSeg[1].c_str());
                        cout << "kicked_userID=" << kicked_userID << endl;
                        string msg = "User "+to_string(kicked_userID)+" logout\n";
                        eraseUserPipe(userPipeTable, kicked_userID, kicked_userID, CAUSE_EXIST);
                        erase_client_data(kicked_userID);

                        int kicked_ssockfd = ssockFd_userId(kicked_userID, GET_SOCKFD);

                        close(kicked_ssockfd);
                        FD_CLR(kicked_ssockfd, &activeFds);
